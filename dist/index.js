...
const viewDepartments = async () => {
    try {
        const result = await pool.query('SELECT id AS "ID", name AS "Department Name" FROM department');
        const table = new Table({
...
const viewRoles = async () => {
    try {
        const roles = await pool.query('SELECT roles.id AS "ID", roles.title AS "Job Title", roles.salary AS "Salary", department.name AS "Department" FROM roles LEFT JOIN department ON roles.department_id = department.id');
        const table = new Table({
...
const addRole = async () => {
    try {
        ...
        await pool.query('INSERT INTO roles (title, salary, department_id) VALUES ($1, $2, $3)', [roleCreate.title,
            roleCreate.salary,
...
const addEmployee = async () => {
    try {
        ...
        await pool.query('INSERT INTO employees (firstName, lastName, role_id, manager_id) VALUES ($1, $2, $3, $4)', [employeeCreate.firstName,
...
const updateEmployeeRole = async () => {
    try {
        ...
        await pool.query('UPDATE employees SET role_id = $1 WHERE id = $2', [employeeUpdate.role, employeeUpdate.employee]);
